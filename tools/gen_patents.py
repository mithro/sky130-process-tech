...
def scope_and_completeness(fams: list[dict]) -> list[str]:
    ...
    return [
        ...
        "reclassification targets the first sweep's own review found necessary) -- systematic by "
        "classification rather than by guessed keywords, per this index's own stated goal. It found 559 "
        "distinct families across 587 assignee-restricted hits, corrected SkyWater's indexed name to "
        "\"Sky Water Technology Foundry, Inc.\" (two words -- the keyword sweep's \"skywater\" spelling "
        "had matched nothing), and added 81 more families plus one new member (a just-granted patent) of "
        "an existing SkyWater family. The Spansion estate was separately re-triaged on a coordinator "
        "decision that it is not process lineage (the 2015 Cypress-Spansion merger is ownership, not fab "
        "lineage): a systematic check found every Spansion technique the estate covers already has a "
        "lineage or expired example elsewhere in this index, so all 91 still-deferred Spansion families "
        "were marked out of scope rather than added. Of the round-4 Weebit Nano backlog and this round's "
        "own supplementary Weebit CPC sweep, only one further family (a device/process patent on ReRAM "
        "retention) was added; the rest remain circuit-level programming/read/write/sensing schemes, out "
        "of scope by this index's own exclusion of circuit patents. Both sweeps, every family's triage "
        "decision and the reason for each are recorded in the repository at "
        "`docs/plans/patent-discovery-log.md` (outside the built site). What is left: the Cypress CPC "
        "sweep's own \"in scope, left for later\" residue is empty (every hit was triaged to a decision "
        "this round), but neither sweep looked beyond these five core assignees, and no attempt has been "
        "made to find a PPUBS-sourced family's other-jurisdiction or other-US-member siblings.",
    ]
