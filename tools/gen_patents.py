...
def scope_and_completeness(fams: list[dict]) -> list[str]:
    ...
    return [
        ...
        "were added. A second sweep on 2026-09-20 dropped the title-keyword restriction and queried the "
        "same core assignees (Cypress, SkyWater, Longitude, Infineon Technologies LLC, Ramtron) by CPC "
        "class instead (H01L21/23/27/29, H10B, H10N70, G03F, C23C, C30B, plus the H10D/H10P/H10W "
        "reclassification targets the first sweep's own review found necessary). It found 559 distinct "
        "families across 587 assignee-restricted hits, corrected SkyWater's indexed name to \"Sky Water "
        "Technology Foundry, Inc.\" (two words -- the keyword sweep's \"skywater\" spelling had matched "
        "nothing), and added 81 more families plus one new member (a just-granted patent) of an existing "
        "SkyWater family. Of the 559, 387 were rejected by a scripted title-keyword classifier rather than "
        "individually reviewed; a later audit found the classifier's vocabulary list, not its underlying "
        "CPC-class sweep, was the weak link, and its default reject bucket was materially wrong (see the "
        "next paragraph). The 91 Spansion families the round-4 sweep had left undecided were re-triaged on "
        "a coordinator decision that Spansion is not process lineage (the 2015 Cypress-Spansion merger is "
        "ownership, not fab lineage): each row names the specific existing index entry that already covers "
        "its technique. Two rows (process/UV-induced-charging damage protection during BEOL processing) "
        "briefly had no covering entry and were held as the coordinator's own named exception, but "
        "this round's own H1 re-triage of the default bucket (see the next paragraph) then added two "
        "same-lineage-assignee Cypress families of that identical technique class, so the exception's "
        "premise no longer held; both rows were re-decided out of scope, now naming those two Cypress "
        "families as their covering entries. Of the round-4 Weebit "
        "Nano backlog and that round's own supplementary Weebit CPC sweep, only one further family (a "
        "device/process patent on ReRAM retention) was added; the rest remain circuit-level programming/"
        "read/write/sensing schemes, out of scope by this index's own exclusion of circuit patents. The "
        "Saifun/Cypress Semiconductor Ltd. NROM estate (the Netanya line, merged into Spansion in 2008) is "
        "excluded by this same Spansion decision, not overlooked: it is a real gap in the sense that no "
        "public source disproves it belongs, but the coordinator's ruling applies to it identically.",
        "",
        f"A later audit (round 6) found the classifier's title-keyword vocabulary rejected roughly a "
        "third of the 387 by default without any individual review, including families whose titles used "
        "a synonym or a different granularity of an in-scope phrase (\"contact openings\" for \"contact "
        "structure\", \"planarized structure\" for \"CMP\", and so on) and at least one family (a method of "
        "ONO integration into a logic CMOS flow) a previous round had already judged in scope before the "
        "classifier silently reversed it. Every one of the 387 rejected rows was re-read this round from "
        "that family's own USPTO Patent Public Search full-text title and abstract (not the classifier's "
        f"title-only heuristic): {plural(h1_added, 'family')} were found in scope and added -- process, "
        "device-structure and equipment/metrology technique classes across CMP, etch, deposition, "
        "implant, oxidation, lithography, strip, contact/interconnect, STI, NVM device structures, and "
        "process-induced-charging protection -- and the remaining rejections were confirmed out of scope "
        "with a reason specific to that family's own abstract (ferroelectric/FRAM material, packaging/"
        "assembly, circuit or system-level content, EDA/software tooling, or a memory technology -- MRAM, "
        "FinFET, 3-D NAND -- no public source shows this lineage's planar-CMOS PDK offering). A handful of "
        "rows turned out to be the same DOCDB family as one already decided elsewhere in this round (found "
        "again under a different assignee query) and are recorded as such rather than duplicated. One "
        "family's abstract could not be fetched after repeated retries (a persistent server error) and was "
        "decided from its title and CPC classes alone, consistent with the already-validated keyword-"
