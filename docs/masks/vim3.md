...
On the {ref}`VIM3 <step-144>` page's
reading, the {ref}`VIM3E <step-145>` etch cuts
the holes through the inter-metal oxide down to two kinds of floor, the
metal-3 lines and the capacitor plates. The mask is the second of the three via
...
Inside `areaid.mt`, the "Location of e-test modules within the
frame", rule via3.1a allows "Two sizes of square via3 allowed inside
areaid.mt: 0.200um and 0.800um".[^pdk-periph][^pdk-06] The PDK's
...
data", checks it as "via size inside module cut must be 0.200 OR
...
dielectric thicknesses (inference).

The `capm` rules that govern the via
...
described here (inference).[^pdk-periph]

The published SKY130
{term}`test tile` names the structures that exercise the placement:[^raw-data-testtile-pads]

* a
  "Via-3 array, M4-M3 near CAPM (min via-3 space to CAPM)" with a row
  "For Via Shorts, 1120 vias, each in 2x2 arrays inside holes in CAPM"
* a "Via-3 chain, 5754
  via-3, M4-CAPM over M3 (min via enclosure)"
* a "Kelvin via-3, M4-CAPM
  over M3"
* off the capacitors, an "s8p via3 (0.2x0.2) contact chain
  (16500 contacts)"

The test tile's second large capacitor over metal 4, "Same
...
The [*Error Messages*](<https://skywater-pdk.readthedocs.io/en/main/rules/errors.html>) page lists two checks named
...
"Via3-PLM" and the info text "Via 3 (M3→M4)".[^mask-renders]
...
that every layout joins metal 3 to metal 4.
...
* **Plate number.** The sheet does not say what `575` encodes.

  `575` falls
  between `572` for `CAPM` and `580` for `MM4`.[^steps-sheet] From `500` to `590` the
...
The tab does not define its codes, give a unit for
...
and reads the digit 4, less certainly,
as the 4× reduction ratio.
...
6% transmission MoSiON absorber".[^photronics-abr] That statement describes one
...
same type code for `VIM2` carries no spot size.
...
to 10 nm".[^abboud-1998] Kurihara et al. compared e-beam and laser
...
optimised such a mask for contact holes.[^chen-1999-psm]
...
### Two floors and the resist

On the step pages' readings the resist
...
the substrate reflectivity.[^brunner-1991]
...
For holes Yamamoto et al. describe hierarchical
...
Metal 3 must enclose the via by 0.060 µm (via3.4) and by
...
({ref}`overview-metal-cap`) and to the capacitor plates. On the step pages' readings
the resist is stripped within that step.
...
rules x.1b, x.2, x.7, x.15a and x.18.
...
:::{table} The `via3` rules, the metal-4 enclosures of via 3, the `capm` rules the step page reads for via 3 and the mask-data rules, as published
...
Table 2 of *Criteria & Assumptions* repeats the size and space for "Via3-PLM", and gives a "Via3-Cu" row for a back end SKY130's aluminium flow does not
use:[^pdk-03]
...
| `VIM3CD` | size | 0.2 |
| `VIM3CDSP` | space | 0.2 |
| `VIM3_CuCD` | "Via3-Cu" | 0.21, the value of the copper rule via3.11[^pdk-periph] |
| `VIM3_CuCDSP` | "Via3-Cu" | 0.18, the value of the copper rule via3.12[^pdk-periph] |
:::

The summary Tables F3c and F3d stop at metal 3
...
For the plate the decisive figures are one 0.200 µm square on a
0.40 µm pitch (our arithmetic from via3.1 and via3.2).

## Related pages

* **Steps.** {ref}`VIM3 <step-144>` and {ref}`VIM3E <step-145>` — the mask step and
...
* **Masks.** {ref}`mask-mm3` and {ref}`mask-capm` — the metal-3 lines and capacitor
