...
`masks.csv` lists two pad masks with
the one acronym, "Pad (scribe protect)", marked as used in SKY130, and
"Pad (scribe unprotect)", unmarked. The process-steps sheet records
...
2 µm and a product pad tens of micrometres. The design rules that
shape them sit less in the periphery rules than on the PDK's Error
Messages page and in its polyimide and packaging criteria.
...
as "pad.mk with width > 100um".[^pdk-06] `gds_layers.csv` lists beside `pad`:[^pdk-06]

* a
  `padCenter` marker at 81:20, "Pad center marker"
* an `areaid.fe`
  identifier at 81:3, "Pads in padframe identifier"
* a `target` layer
  at 76:44, "Metal fuse target", on the same layer number as
  `pad`

The periphery rules give the opening only two rules, a
...
for GDS data".[^pdk-errors] It has many more pad checks than the periphery rules:[^pdk-errors]
...
* it
  ...
  "pad opening inside inductor by met5" (cupad.3)
* it gives a bond pad
  ...
  "8.8 Max length of 45 degree bevel on Bond pad" (pad.13)
* it gives a "150.0um Max
  ...
  seal ring)" (pad.15)
* it words pad.3 as "hugepad with area less than
  30000.0 not allowed"

The {ref}`PDM <step-168>` page reads
...
* "Etest pad Width and Length is either 67.00 um or 35.00um"
  (scribe.9)
* a spacing of 90.5 µm between 67 µm pads (scribe.10)
* "2.50um
  min. enclosure of Etest pad by met1" and the same for metals 2 to 5
  (scribe.13)
* "Pad.dg is to be drawn from 3.0um from scribe edge to 13.0um
  from scribe edge" (scribe.20)
* "Scribe must not enclose pdm.dg nor
  pdm.mk except for etest pads, die pad rings" (scribe.21)
...
(inference from the names and the mark). Rule scribe.21, which keeps pad
...
gives in its column headed "CD":[^pdk-03]

* an "Enclosure of bondpad by polyimide
  (YUY-165)" of 0.5 (`PimPadEnc`)
* enclosures of `pad:dg` by PBO of 4.5
  (`PBOPadEnc`, "inside inductor capture pad, with DECA online
  monitoring") and 7.5 (`PBOPadEncDECA`, "per standard DECA rules")
* a "Min
  Pim width over pad openings" of 87 (`PimOverPad_CD`)
* a "Min/Max
  enclosure of pad.dg inside M5RDL by pmm" of 0 (`pmmM5RDLpadEnc`)

In
its column headed "space" Table 9 gives a "Min spacing of pmm to (rdl NOT (pad.dg sized
by 0.5))" of 19.16 (`pmmRDLspc`), which Table 10 repeats.[^pdk-03] Table 7 gives a
"Min enclosure of pad by pmm for Cu inductor (JNET-80)" of 0
(`padPMMEncInd`).[^pdk-03]

The DECA table of the *WLCSP Rules* requires a
...
publishes no operation from `pad` to the plate.
...
from the nikon cross in the layout".[^pdk-errors] It also checks "0.635 min. spacing
...
without text label" (scribe.6a).[^pdk-errors]

The page does not say what
...
cut at 45°, less a cross of three rectangles, 2.4 µm across with 0.5 µm arms (our reading of the GDS
file).[^caravel-sealring] That is the opposite form from
...
AND areaid.mt) sized by cpdm.4".[^mask-renders]

The note and the render
...
source for the note.[^pdk-periph][^pdk-errors]

The note and the choice of
...
MPW-1).[^mask-renders] On each run most dies carry exactly 96 or exactly 107: 36 of
the 40 dies on MPW-1 (all 107), 40, 40, 39, 39, 40, 38 and 39 on MPW-2 to
MPW-8 in turn.[^mask-renders] The other dies carry from 106 to 284 901
shapes.[^mask-renders]

We read the two repeated counts as layouts common
...
show the drawn layouts, not the plates.[^mask-renders][^steps-sheet]
...
At 4× a 2 µm opening is 8 µm on the plate (our
...
The {ref}`PDM <step-168>` page reads a positive DNQ/novolac
...
The opening defines the part of the pad that probe needles and wire
...
:::{table} The `pad` rules and the other rules that name the pad opening or its mask data, as published
...
the polyimide and PBO criteria quoted above.[^pdk-03]
...
* **Steps.** {ref}`PDM <step-168>` and {ref}`PDME <step-169>` — the mask step and
...
* **Materials.** {ref}`material-lithography-materials` — resists, developer and
  reticles.
* **Masks.** {ref}`mask-mm5` — the mask of the pad metal; {ref}`mask-nsm` — the other
