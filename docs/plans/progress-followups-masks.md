# Progress: fixing the round-1 review (not tracked in the repository) findings on `topic/followups-masks`

Checked by grep across the whole `docs/` tree on this branch, 2026-09-18.
The previous (interrupted) fixer's commits — since rewritten by a rebase,
so identified here by subject rather than by the now-stale hashes this
file originally recorded — turn out to have covered every item below,
