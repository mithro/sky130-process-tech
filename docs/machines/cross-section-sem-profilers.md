...
How deep
is the trench, is the gap between two lines filled without a void?
Does
...
imaged in a scanning electron microscope.
...
are non-destructive and can run in line.
...
crystal planes".[^sela-2001] Analysts then polish the cleaved face until it reaches the
...
signals".[^hitachi-s4800] It could also carry an energy-dispersive X-ray
...
**Dual-beam tools.** A dual-beam system combines an ion column with an electron column, so that
...
that no permanent marks are left on the surface".[^bennett-1981]

**Tencor's production profilers.** Tencor's
...
applications".[^tencor-hrp200-1996]

**Later tools.** By 2002 KLA-Tencor's HRP-240/340 added
...
**Imaging sidewalls.** Martin and Wickramasinghe imaged sidewalls with "a special boot-shaped
...
:::{table} Representative cross-section, FIB, profiler and AFM tools of the 200 mm era (figures as each source gives them)
:widths: 16 20 8 56
...
:::{table} How this reference reads the Physical Analysis entries
:widths: 26 50 24

| Entry as listed | What it names | Status |
|---|---|---|
| "FEI Dual Beam FIB/SEM" | a dual-beam focused-ion-beam and SEM system of the kind FEI sold as DualBeam[^fei-dualbeam-2006] | our reading |
| "Hitachi S-4800" | Hitachi's field-emission SEM of that name[^hitachi-s4800] | our reading |
| "SELA EM2 Precision Cleave" | a SELA cleaving system, a company whose automated cleaving tools prepare SEM and TEM samples,[^sela-2001] but no description of an EM2 was retrieved | our reading |
| "Oxford PlasmaLab RIE deprocessing" | a layer-removal tool | our reading |
| "Allied TechPrep polisher" | a mechanical-preparation tool | our reading |
:::
...
the capabilities page names no step, and no step page assigns them.
...
* **Isolation.** {ref}`STIE <step-006>` names cross-section SEM with a
  ...
  The {ref}`HDP-CVD page <machine-hdp-cvd>`
...
* **Machines.** {ref}`machine-cd-sem-overlay-metrology` — top-down CD
  ...
  thickness gauges after deposition and CMP.
