...
* **Does:** polishes the {ref}`NILD4 <step-126>` oxide flat, stopped
  by removal amount at a controlled height above the metal-2 lines.
* **Why:** a subtractive aluminium back end without planarisation
  accumulates topography with every level.
* **Public numbers:** via-2 height 0.42 µm;[^pdk-04] minimum oxide
  pattern density 0.7 (m2.pd.1) in 700 µm windows.[^pdk-periph]
* **Likely SkyWater tool:** Applied Materials Mirra CMP — **strong**
  (the tool and its oxide polish); assignment to this step is an
  **inference**.[^skw-01]
* **Not public:** the slurry, pad, removal amount, target oxide
  thickness and endpoint method (→ Open questions).
:::
...
As at {ref}`CMPM <step-116>` it is an *oxide-on-oxide*
...
:::{figure} /_static/figures/beol-127-cmpm2.svg
:alt: Two cross-sections of the upper films, one above the other, cut off at the bottom. Before the step a thick pale oxide buries three metal lines, its top higher over the lines than over the gaps. After it the oxide is polished flat, leaving a layer of it over the tops of the lines.
:width: 560px
:name: fig-beol-127-cmpm2

Before, the NILD4 oxide as deposited; after, polished flat until the topography is gone, at a height above the metal-2 tops. The polish is stopped by the removal amount, not by a stop layer, and how much it removes is not public. The oxide left over metal 2 is drawn a little thicker than the oxide over metal 1; on the page's reading, with the cap of the next step it makes up the PDK's via-2 height of 0.42 µm over metal 2.[^pdk-04] Nothing is drawn to scale, and the pattern-density steps the page describes are not drawn. The lower part of the slice is cut off: the drawing starts inside the inter-level oxide under metal 1. The bottom and cap films of the metal-2 lines, the via plugs and their liners, the oxides of the via level and the metal-1 lines with their contact plugs are drawn but not labelled. Not to scale.
:::
...
"oxide" first among its applications.[^skw-01]

The PDK's rules for
metal 2 are written around this polish exactly as metal 1's are
around the first:

* a "Min MM2_oxide_Pattern_density" of 0.7 (m2.pd.1)
  ...
  levels;[^pdk-periph]
* the assumptions table gives an "Oxide Bias for
...
* **A uniform via-2 depth.** The {ref}`VIM2E <step-130>` etch must
  ...
  the metal-2 cap where the oxide is thinner
  ({ref}`overview-metal-cap`); the post-polish thickness
...
3. **Recipe.** Removal follows Preston's law,[^preston-1927] modified
   ...
   review the models.[^nanz-1995]

   Down-force of a few psi and platen
...
4. **Endpoint.** By removal time calibrated on monitors, or by
   ...
   would scratch and thin the lines. The thicker target here
...
6. **Metrology.**
   ...
   - Remaining oxide thickness over metal-2 test pads by
     reflectometry or ellipsometry at many sites;
   - die-level thickness
     maps on density test structures (Stine et al.'s
     method[^stine-1998]);
   - scratch and particle inspection on the KLA AIT class (our reading of "AIT" in
     SkyWater's job posting);[^job-06]
   - via-2 chain resistance
...
* **{ref}`Silica-based oxide CMP slurry <material-cmp-consumables>`** (KOH- or NH₄OH-stabilised fumed
...
* Previous: {ref}`NILD4 <step-126>` (the film).
* Next:
  {ref}`NCAPOX4 <step-128>` (the cap), then {ref}`VIM2 <step-129>`.
* Depends on: the topography it removes, {ref}`MM2E <step-125>`; the fill rules
...
* Feeds: the via etch that depends on its thickness control,
  ...
  {ref}`MM3 <step-139>`.
* Same category: the same polish at other levels, {ref}`CMPM <step-116>` (where the
...
* [LinkedIn, SkyWater *Defect Technician 2* posting](<https://www.linkedin.com/jobs/view/defect-technician-2-at-skywater-technology-4434994342>) — inspection
  tools.[^job-06]
* [SkyWater PDK, *Periphery rules*](<https://skywater-pdk.readthedocs.io/en/main/rules/periphery.html>) — m2.pd.1 (0.7), m2.pd.2a/b, the
  `cmm2` waffle-drop check; via2.1a, m3.1.[^pdk-periph]
* [SkyWater PDK, *Criteria & Assumptions*](<https://skywater-pdk.readthedocs.io/en/main/rules/assumptions.html>) — "Oxide Bias for MM2" 0.6,
  "Oxide Bias for MM3" 1.15; "Min pattern density for oxide" 0.75;
  "Min MM* PD range" 0.3.[^pdk-03]
* [SkyWater PDK, *Process stack diagram*](<https://raw.githubusercontent.com/google/skywater-pdk/main/docs/_static/metal_stack.svg>) — `met2` 0.36 µm; via2
  0.42 µm.[^pdk-04]
...
* **Slurry, pad and endpoint.** The slurry, pad, removal amount, target oxide thickness over
  metal 2 and the endpoint method of `CMPM2` are not public.
* **Split of the via-2 height.** How the 0.42 µm via-2 height of the PDK[^pdk-04] is split between
  the polished NILD4 and the {ref}`NCAPOX4 <step-128>` cap is not
  public.
* **Oxide Bias.** Why the PDK's "Oxide Bias" rises from 0.6 for MM1 and MM2 to 1.15
