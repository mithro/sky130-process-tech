...
:::{figure} /_static/figures/sd-080-spox.svg
:alt: Two enlarged cross-sections of one transistor edge, one above the other. Before the step a capped gate stands with a nitride block against each sidewall, on a thin oxide over doped silicon. After it a thin new film of even thickness covers everything: the flat oxide, the outer face and top of the nitride block and the top of the gate.
:width: 560px
:name: fig-sd-080-spox
...
* **{ref}`TEOS <material-precursors>`** (liquid, vaporised) with **{ref}`O₂ <material-process-gases>`**, or **silane and
  N₂O**;[^wiki-teos][^wiki-pecvd] helium or nitrogen carrier.
* **{ref}`NF₃ <material-etch-gases>` or CF₄/O₂** chamber clean (PECVD); {ref}`quartz ware <material-hardware-consumables>` and
