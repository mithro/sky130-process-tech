...
step — an oxide polish with a nitride stop.

**Specific to this step:**

* The later polishes in the flow ({ref}`CMPP <step-090>` over poly,
  {ref}`CMPL <step-106>`, {ref}`CMPM <step-116>` over metal levels,
  and the tungsten polishes {ref}`WCMPLI <step-100>`,
  {ref}`WCMP2 <step-111>`) planarise interlevel dielectrics or clear
  metal. Only this one uses a nitride stop and only this one
  determines the height of the silicon islands.
...
* **Applied Materials Mirra CMP**
  ...
  - *Runs this step:* **inference**, from the film, since the list
    names no steps.
* **Post-CMP clean** — SkyWater lists the "SEZ223, Davinci"
  single-wafer tools (HF, DSP+HF), and the Mirra entry also lists
  "Track ammonia clean" and "IPA clean";[^skw-01] the technician
  profile mentions "a SEZ etcher tool"[^skw-07]
  - *Tool exists:* strong for existence.
  - *Runs this step:* a brush scrubber is not named on any public
    page (open question).
* **Defect inspection — KLA AIT / SP1**, our reading of "AIT" and
  "SP1" in a SkyWater job posting's "SEM/AIT/KLA/SP1/EV300/1X"[^job-06]
  - *Tool exists:* medium.
...
* **Slurry type and etch-back.** Whether SKY130 uses a silica or a
  ceria (high-selectivity) slurry, and whether a reverse-mask
  etch-back was ever part of the S8 flow, is not public.
* **Post-polish targets.** The target post-polish nitride thickness
  and the allowed dishing / erosion are not public.
* **Post-CMP clean tool.** The post-CMP cleaning tool is not
  identified on any public SkyWater page.
* **Mirra vendor page unavailable.** Applied Materials' own Mirra
  product pages could not be retrieved during writing (HTTP 403), so
  no vendor description of the tool is cited; SkyWater's capability
  page is the only source used for it.[^skw-01]
