...
:::{figure} /_static/figures/sd-085-nsdm.svg
:alt: Two cross-sections of the wafer, one above the other. Before the step a thin oxide covers everything: both active areas, a narrow capped gate with nitride blocks on its sidewalls, and a wide stack on the oxide-filled trench. After it resist covers the trench and the wide stack and leaves both active areas open.
:width: 560px
:name: fig-sd-085-nsdm
...
* **Positive i-line (DNQ/novolac) photoresist** ({ref}`lithography materials <material-lithography-materials>`), about 1 µm (PDK
