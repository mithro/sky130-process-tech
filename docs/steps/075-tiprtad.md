...
* **Junction position.** ITRS 2001 asks for a 27–45 nm extension
  ...
  The roadmap's rule Xj = 0.55 × physical gate length would put SKY130's tip
  nearer 80 nm for its 0.15 µm drawn gate (our reading; SKY130's physical
  gate length is not published).[^itrs-01] The PDK publishes no depth for
  the tip, only the 0.01 µm in the "Vertical Space" column of its "N Tip
  (As)" row, which we read as the lateral extent.[^pdk-03]
...
* {ref}`Sacrificial polysilicon sidewall process and rapid thermal spike annealing for advance CMOS fabrication <patent-gp24961682>` — US 6,518,136 B2 (2000)
* {ref}`Optimal spike anneal ambient <patent-gp31992738>` — US 6,803,297 B2 (2002)
* {ref}`Advances in spike anneal processes for ultra shallow junctions <patent-gp32659145>` — US 6,897,131 B2 (2002)
<!-- index-links:end -->
