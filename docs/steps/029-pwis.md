...
* **Does:** strips the thick `PWBM` resist after the two P-well
  implants and cleans the wafer for `PWDEM`.
* **Why:** residue would block the drain-extended P-well implants,
  and sputtered metals must be cleaned off before the RTAI anneal.
...
* **Likely SkyWater tool:** GaSonics PEP / Iridia RF microwave /
  ...
  (assignment).[^skw-01]
* **Not public:** the strip recipe and wet sequence (→ Open
  questions).
:::
...
carbonised {term}`crust <implant crust>` and a real {term}`popping` risk.

Boron is light and
has a long range, so it deposits less of its energy in the top of the
resist than phosphorus does, and the crust is somewhat thinner than
after the N-well implants (inference).[^txt-01]

Because `PWBM` is a *block* mask,
...
strippers".[^pat-strip-tsmc]

And it must be removed without popping,
...
1. **Two-stage plasma ash.** First stage below the popping threshold
   ...
   oxygen stage for the bulk.

   Downstream plasma so that neutral atomic
...
| KLA-Tencor SP1 / AIT | medium |

* **GaSonics PEP, Iridia RF microwave, Mattson Aspen II**
  - *SkyWater says:* lists them, the latter two with hydrogen-bearing
    chemistries.[^skw-01]
  - *Tool exists:* strong.
...
* Same category: comparable heavy-implant strips —
  {ref}`DNIS <step-009>`, {ref}`LVTPIS <step-021>`.
...
* [Plummer, Deal and Griffin, *Silicon VLSI Technology*](<https://openlibrary.org/isbn/9780130850379>) — energy
...
* **Strip recipe.** The strip recipe and wet sequence are not public.
* **SC-2 timing.** Whether an SC-2 metal clean is included here or
  deferred to the pre-anneal clean before {ref}`RTAI <step-034>` is
  unknown.
