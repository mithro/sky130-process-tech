...
P-well a two-energy chained implant. No public source describes the
pair; chaining is, however, how every published retrograde P-well of the
...
* **Field isolation.** Under the trench oxide between two n⁺ diffusions
  the doping must be high enough to prevent inversion and punch-through;
...
* **Sequence.** Back-to-back with `PWI` on the same tool where the
  ...
  The two implants share the resist's thermal budget.
