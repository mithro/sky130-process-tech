...
and its strip are in the collapsed note below.
...
:::{figure} /_static/figures/sonos-039-tunme.svg
:alt: Two cross-sections of the wafer, one above the other. Before the step photoresist and a thin anti-reflective film cover the surface except for a window over the right-hand active area, whose bottom is thin oxide over two bands drawn as dashed outlines. After it the resist and the film are gone, the thin oxide is gone from the window so that bare silicon shows there, and the oxide remains on the left-hand active area and to the left of the window.
:width: 560px
:name: fig-sonos-039-tunme

Before, the tunnel-mask resist over its window; after, the oxide gone from the window down to the silicon, and the resist and the anti-reflective coating gone from the whole wafer. The etch is drawn only inside the window, as this page describes a masked etch; whether the oxide is also removed elsewhere after the strip is an open question. The oxide is drawn as the pad oxide, which the page infers. The resist is drawn stripped, with the coating under it, because this page treats the strip as the tail of this step; how the coating comes off is not stated. The punch-through stop and the depletion implant (the bands of PTSI and DEPI) are unchanged: the first is drawn faded in both panels, the second is not labelled in the upper panel and is faded in the lower, and the liner oxide is drawn faded; the anti-reflective coating (upper panel), the P-well and the deep N-well are drawn but not labelled. Not to scale.
:::

This reference treats the resist strip (an oxygen-plasma ash in the
Cypress flow) and the pre-oxidation clean as the tail of this step; they
could equally be the head of the next. The sequence below includes them.
...
The later Cypress patent puts the tunnel oxide at "from about 15
angstrom (Å) to about 22 Å".[^pat-02] The pad oxide is 10–20 nm and the
tunnel oxide 1–3 nm, so a tunnel oxide grown on top of the pad oxide
...
5. **Pre-oxidation clean.** An RCA-type sequence — {term}`SC-1` for particles
   ...
   Cypress ranges, and the patents' own SC-1 and SC-2 conditions and
   ozonated-water variant, are in the collapsed note below this list.
   The room-temperature alternatives to the hot RCA sequence are
   reviewed by Ohmi.[^ohmi-1996]
...
:::{dropdown} From patents shown as in force (US 2009/0179253, estimated expiry 2027-06-17; US 8,093,128, estimated expiry 2028-10-22; US 8,796,098, estimated expiry 2034-02-26) — open to read
The tunnel dielectric is 1.0–3.0 nm in the thermal-oxidation embodiment
...
* **{ref}`Automated wet bench <machine-wet-bench>`** with dilute-HF/BOE, SPM, SC-1 and SC-2
  tanks and a rinser/dryer (Akrion, DNS/SCREEN, SCP), or a
...
* **Hydrofluoric acid (49 %)** ({ref}`wet chemicals <material-wet-chemicals>`) diluted to 50:1–100:1, or **buffered
  oxide etch** (NH₄F/HF) with **surfactant**[^wiki-boe] (the collapsed
  notes above); {term}`BOE` is on SkyWater's Akrion list.[^skw-01]
* **Sulphuric acid and hydrogen peroxide** (SPM), **ammonium
  hydroxide** (SC-1), **hydrochloric acid** (SC-2),[^wiki-rca]
  **ozone** if an ozonated-water clean is used (the collapsed note
  above).
