...
* **Does:** etches the ONO stack away from everywhere except the
  memory transistors, leaving it as islands.
* **Why:** it separates memory from logic — after it, the logic
  silicon can be cleaned and oxidised for ordinary gate oxides.
* **Public numbers:** none published for SKY130.
* **Likely SkyWater tool:** Lam 9400 TCP — strong (tool); inference
  (assignment).[^skw-01]
* **Not public:** whether the bottom oxide is cleared here or at the
  gate-oxide pre-clean (→ Open questions).
:::
...
the memory cells and from the field.

It leaves the {term}`ONO` only as
...
Before, the resist island on the stack; after, the stack left as one island over the memory transistor, and the silicon bare everywhere else. The top oxide and the nitride are drawn etched away down to the oxide beneath them, which the page infers to be the pad oxide; that oxide is then drawn cleared as well, and the resist stripped, because this page treats the resist strip and the pre-gate-oxide clean as part of this step, although whether the last oxide goes here or at the GOX100 pre-clean is not stated publicly. The loss of fill oxide in the etches is not drawn; the fill oxide and the pad oxide left under the edge of the island are drawn but not labelled. The punch-through stop and the depletion implant (the bands of PTSI and DEPI) and the liner oxide are drawn faded; the P-well and the deep N-well are drawn but not labelled. Not to scale.
:::

Precisely: the {term}`blocking oxide` and the
nitride or {term}`oxynitride` trapping layer, and any sacrificial cap, are removed by a plasma etch that
stops on the underlying oxide. This is, we infer, the pad oxide from
{ref}`BOX <step-002>`, still present outside the tunnel
windows. The last oxide is then cleared, either here or at the
gate-oxide pre-clean, by a wet etch.

Both patents are shown as in
...
* It runs on the Lam 9400 / DPS "poly/nitride" class of tool (the
  assignment is inferred — see below).
* Its distinctive difficulty is
  ...
  of every logic transistor.
* The etch is therefore run like a
  {term}`spacer` etch, a nitride etch with "selectivity to oxide" that
  must "stop on a thin oxide without trenching the silicon", rather
  than like a {term}`hard-mask <hard mask>` open ({ref}`category-etch`).
...
* **The ONO edge must be clean.** The island sidewall is the boundary
  along which the logic gate oxide will later grow.
...
2. **Top oxide.** The blocking oxide and any sacrificial cap are
   opened in a fluorocarbon plasma (CF₄/CHF₃ with O₂ or Ar).

   The
   Cypress thicknesses are in the collapsed note below this list. This
   is the chemistry that "etches oxide as SiF₄ only under ion
   bombardment" ({ref}`category-etch`, citing Flamm and
   Donnelly[^flamm-1981]).
3. **Nitride.** The trapping layer
   is etched in CF₄/O₂,
   CHF₃/O₂ or SF₆-based chemistry tuned for {term}`selectivity` to the oxide
   below.

   (The two Cypress ranges are in the
   same note.) A 1997 ASMC paper reports a nitride spacer etch "with high
   ...
   being etched".[^pat-cn-tel] This is followed by a short {term}`over-etch` that
...
5. **Bottom oxide.** The remaining pad oxide (inferred; plus the
   fraction of a nanometre that the tunnel oxidation added[^deal-1965])
   is removed in {term}`BOE` or dilute HF, exposing the logic
   silicon for gate oxidation.

   BOE or dilute HF is the etchant the Cypress
   patent names (collapsed note below this list). Whether this happens here or as the
...
6. **All-wet alternative.** The stack could in principle be removed
   ...
   describe the dry-then-wet route instead.
...
| Applied Materials DPS II | strong (existence); **medium** (assignment) |
| Lam 4400 | strong (existence); **weak** (assignment) |
...
* **Applied Materials DPS II**
  - *SkyWater says:* lists "HBR, Cl2, NF3, CF4, CHF3, O2 – gate,
    trench, W/WN".[^skw-01]
  - *Tool exists:* strong for existence — has CHF₃ for the oxide
    layers.
  - *Runs this step:* **medium**, since its CF₄ and CHF₃ etch nitride
    although the entry names no nitride application.
* **Lam 4400**
  - *SkyWater says:* lists "HBr, Cl2, C2F6, CF4, SF6, O2".[^skw-01]
...
* **Akrion Gamma batch wet bench**
  ...
  - *Tool exists:* strong for existence — for the BOE clear (and the
    phosphoric route, if used).
...
* Next: {ref}`GOX100 <step-043>` (first gate oxidation on the cleared
  silicon).
* Depends on: the stack being removed was formed at
  {ref}`ONO <step-040>`.
* Same category: the same nitride chemistry serves the {term}`STI`
  hard-mask etch {ref}`STINITE <step-005>` and the spacer etch
  {ref}`SPE <step-077>`.
* Feeds: the ONO islands are etched again, self-aligned to the gate,
  at the poly etch ({ref}`P1ME <step-062>`).
...
* **Oxide identity.** That the oxide under the resist/nitride is the
  ...
  public.
* **Nitride removal method.** Whether the nitride is removed dry or
  wet, and on which tool, is inferred.
* **Where the bottom oxide is cleared.** Whether the bottom oxide is
  cleared here or at the {ref}`GOX100 <step-043>` pre-clean is not
  stated publicly.
* **ARC use.** Whether an ARC is used on the ONO mask and opened in
...
* **Where the resist is stripped.** Where the resist is stripped is
  ...
  etch.
* **Endpoint and loss.** Endpoint method, over-etch and the resulting
  pad-oxide loss are not public.
