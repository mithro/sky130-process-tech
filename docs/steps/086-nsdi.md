...
:::{figure} /_static/figures/sd-086-nsdi.svg
:alt: Two enlarged cross-sections of one transistor edge, one above the other. Before the step a capped gate with a nitride block and a thin oxide on its sidewall stands over a shallow doped layer and a hatched region. After it vertical arrows come down, and a deeper doped region lies in the silicon from the left edge of the view up to the outer foot of the sidewall films; the shallow layer remains under the sidewall films.
:width: 560px
:name: fig-sd-086-nsdi
