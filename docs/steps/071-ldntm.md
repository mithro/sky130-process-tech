...
the memory cells. Like {ref}`NTM <step-064>` it selects transistors
rather than defining junction edges — the gate does that — and its
features are cell-sized. It differs from the other tip masks in
covering a small fraction of the die: only the memory array and any
SONOS test structures are opened.
...
* **Junction voltage.** During programme, erase and inhibit the
  ...
  body — far more than the 1.8 V a logic tip sees. A heavily doped,
...
* {ref}`US 6,759,288 B2 <patent-gp24577983>` — Double LDD devices for improved DRAM refresh (2000)
...
* The resist thickness and the tilt at which the lightly doped tip is
  ...
  parameter.[^pdk-03]
* The junction-leakage and disturb arguments above are our reading of
  the cell's published operating voltages;[^pdk-07] no public source
  states why the SONOS tip is lightly doped.
* No public source places `LDNTM` on a particular exposure tool.
