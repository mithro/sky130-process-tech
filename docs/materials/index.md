...
| `btbas` | BTBAS (bis(tertiary-butylamino)silane) | {ref}`Precursor (liquid) <material-precursors>` | Low-temperature LPCVD nitride option for the spacer and the {term}`local-interconnect <local interconnect>` cap. | "LPCVD BTBAS low temp nitride".[^skw-01] | {ref}`076 <step-076>`, {ref}`104 <step-104>` |
| `sif4` | Silicon tetrafluoride (SiF₄) | {ref}`Precursor <material-precursors>` | Fluorinated oxide option for the metal-1/metal-2 and metal-2/metal-3 dielectrics. | typical (not listed) | {ref}`115 <step-115>`, {ref}`126 <step-126>` |
...
| `ozone` | Ozone (O₃) for TEOS oxide deposition | {ref}`Precursor; oxidant <material-precursors>` | O₂/O₃ oxidant for the TEOS PSG route. | typical (not listed) | {ref}`089 <step-089>` |
