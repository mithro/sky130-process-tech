...
* **Wet chemicals.** HF and buffered oxide etch ({term}`BOE`); the
...
| Contact liner Ti/TiN (and titanium {term}`silicide`) | {ref}`TI/TIN1 <step-097>`; silicide formed at {ref}`CSIL <step-098>` | none | — | IMP Ti and TiN of the order of 10–30 nm each (typical)[^koerner-1993] |
